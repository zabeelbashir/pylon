...
class Filter_Functions():
	...
	def __init__(self):
		...
		self.dict = {'NetID' : '' , 'DevID' : '' , 'Dev_Type' : '' , 'Type' : '' , 'Priority' : '' , 'PDU' : '' , 'ERRC' : '' , 'SEQ' : '' , 'LQI' : '' }
		self.json_obj = None
		self.json_obj1 = None
	...
	def filter_action(self):
		try:
			if self.flag == 1:
				...
				self.dict['PDU'] = str(self.hex_str(self.line[9:-3]))
				print self.dict['PDU']
				print 'Message Type conversion is:' + self.dict['Type']
				self.dict['LQI'] = self.line[-3:]
				...
				self.json_obj1 = self.dict['NetID'] + ':' + self.dict['DevID'] + ':' + str(self.dict['Dev_Type']) + ':' + '15' + ':' + str(self.dict['LQI'])
				self.logger_obj.log ("JSON Formated: " + self.json_obj1)
				self.line = None
			else:
